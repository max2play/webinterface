...
<script type="text/javascript"
	src="/js/reloadprogress.js?v=<?php echo $service->info->version?>"></script>
<link rel='stylesheet' id=''
	href='/style-new.css?v=<?php echo $service->info->version?>'
		...
		<header>
			...
			<div class="chooseplayer">
				<?php if($service->info->devices) { ?>
					...
					style="width: 84%; margin-bottom: 5px;"
					onChange="window.open(this.options[this.selectedIndex].value,'_self');">
						...
						value="http://<?php echo $device['ip'] ?>"><?php echo $device['name'] ?> (<?php echo $device['ip'] ?>)</option>
						<?php }?>		
					</select> <a
					...
					href="<?php echo "//{$_SERVER['HTTP_HOST']}".strtok($_SERVER["REQUEST_URI"],'?').'?refreshDeviceList=1'; ?>">
				...
				<span class="headerinfo">					
					...
					<?php if($service->checkLicense(true, true) == true){ ?>
						<div class="ui-state-disabled ui-corner-all"
					...
					<?php } else { ?>
						<a href="/plugins/max2play_settings/controller/Basic.php"
		...
		<div id="main" class="wrapper">
			<div id="primary" class="site-content">
				<div id="content" role="main">
					<a
						href="<?php echo "//{$_SERVER['HTTP_HOST']}".strtok($_SERVER["REQUEST_URI"],'?'); ?>"
						class="button-small ui-state-default ui-corner-all"
						style="float: right; background-image: none; background-color: #fff;"
						title="Reload Page"><span class="ui-icon ui-icon-refresh"></span></a>