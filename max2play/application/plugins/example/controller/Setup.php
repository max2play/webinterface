...
class Exampleclass_Setup extends Service
{
     ...
     * e.g.
     * Call Scripts and return Output to view
     */
    private function _doSomething()
    {
        ...
        $output2 = nl2br($this->writeDynamicScript(array(
            'ls /opt/max2play/'
        )));
        ...
        if ($ajax == 0) {
            ...
            if ($this->getProgressWithAjax($progressfile, $create = 1, $reloadWhenFinished = 1, $lastlines = 0, $message = _t("Install Something Started"))) {
                $shellanswer = $this->writeDynamicScript(array(
                    $this->scriptPath . "install.sh >> " . $progressfile . " 2>&1 &"
    ...
    {
        $this->view->message[] = $this->formatMessageOutput($string = $this->writeDynamicScript(array(
            $this->scriptPath . 'myscript.sh'
        )), $hide = true);
        return true;
    }
