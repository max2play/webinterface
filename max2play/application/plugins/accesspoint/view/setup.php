	...
	<form action="" method="get">
		...
		<?php if(!$accesspoint->view->installed){ ?>
			...
			<span class="ui-icon ui-icon-wrench"
				style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _t("Set up an accesspoint on this device") ?></b>
		</p>
		...
		<p class="ui-state-default ui-corner-all"
			...
			<span class="ui-icon ui-icon-cancel"
				style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _t("Remove accesspoint from this device") ?></b>
		</p>
			...
			value="<?php echo _("uninstall") ?>" /> <br />
		<br />
		<p class="ui-state-default ui-corner-all"
			style="padding: 4px; margin-bottom: 1em;">
			<span class="ui-icon ui-icon-wrench"
				style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _t("Edit basic accesspoint settings") ?></b>
		</p>
		<div class="optiondetails">
			<table class="settings">
				<tr>
					<td><?php echo _t("SSID") ?></td>
					<td><input type="text" id="ssid" name="ssid"
						value="<?php echo $accesspoint->config->ssid ?>" /></td>
					<td><?php echo _t("The name of this WiFi network") ?></td>
				</tr>
				<tr>
					<td><?php echo _t("Passphrase") ?></td>
					<td><input type="text" id="passphrase" name="passphrase"
						value="<?php echo $accesspoint->config->passphrase ?>" /></td>
				...
				<tr>
					...
					<td><select name="driver">
							...
							<option value="nl80211"
								<?php if($accesspoint->config->driver == 'nl80211') echo 'selected'?>>nl80211</option>
					</select></td>
				...
				<tr>
					...
					<td><input type="checkbox" id="standalone" name="standalone"
						value="1"
						<?php if ($accesspoint->config->standalone) echo "checked"; ?> />
					</td>
...
foreach ($accesspoint->view->debug as $key => $debug) {
    echo "#### " . $key . " ####\n";
    echo $debug . " \n\n"; 
		 }?>
	</textarea>
