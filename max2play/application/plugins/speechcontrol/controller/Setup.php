...
class Speechcontrol_Setup extends Service
{
    ...
    private function _startJasper($ajax = 0)
    {
        if ($ajax == 0) {
            ...
            if ($this->getProgressWithAjax('/opt/max2play/cache/jasperoutput.txt', 1, 0, 20)) {
                $shellanswer = nl2br($this->writeDynamicScript(array(
                    'sudo -u pi -H /opt/jasper/jasper.py > /opt/max2play/cache/jasperoutput.txt 2>&1 &'
                )));
            }
        } else {
    ...
    private function _install($ajax = 0)
    {
        if ($ajax == 0) {
            ...
            if ($this->getProgressWithAjax('/opt/max2play/cache/jasper.txt', 1, 0, 50)) {
                $shellanswer = nl2br($this->writeDynamicScript(array(
                    $this->scriptPath . 'install_jasper.sh ' . $this->scriptPath . ' >> /opt/max2play/cache/jasper.txt &'
                )));
            }
        } else {
            // Get only last 20 Lines
    ...
    private function _uninstall()
    {
        $this->view->message[] = nl2br($this->writeDynamicScript(array(
            $this->scriptPath . 'uninstall_jasper.sh'
        )));
        return true;
    }
    ...
    private function _readText()
    {
        ...
        $this->view->message[] = $this->writeDynamicScript(array(
            '/usr/bin/espeak -v ' . $_GET['readlang'] . ' -s ' . $_GET['readspeed'] . ' "' . strip_tags($_GET['readtext']) . '" --stdout | aplay -D ' . $_GET['readdevice']
        ));
        return true;
    }
