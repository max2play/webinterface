...
<div class="entry-content">
	...
	<form action="" method="get">
		...
		<input type="button" id="disableLEDBlink" name="disableLEDBlink"
			onclick="document.getElementById('action').value='disableLEDBlink';submit();"
			value="<?php echo _("Disable LED-Blink") ?>" /> <br /><?php echo _("Set LED Blink of ODROID to 10 secondes - Press Button again to set LED-Blink back to original settings.") ?>	
		...
		<input type="submit"
			onclick="document.getElementById('powerbutton').style.display='';return false;"
			value="<?php echo _('Edit Power Button Options') ?>" />
		<div id="powerbutton" class="optiondetails" style="display: none;">
				...
				<tr>
					...
					<td style="width: 25%;"><select name="powerbuttonshort"
						style="width: 90%;">
							<option
								<?php if($as->powerbutton['shortpress'] == 'shutdown') echo 'selected'; ?>
								value='shutdown'><?php echo _('Poweroff Max2Play') ?></option>
							<option
								<?php if($as->powerbutton['shortpress'] == 'xbmc') echo 'selected'; ?>
								value='xbmc'><?php echo _('Toggle XBMC / Squeezelite') ?></option>
							<option
								<?php if($as->powerbutton['shortpress'] == 'myscript') echo 'selected'; ?>
								value='myscript'><?php echo _('Start my own Script') ?></option>
					</select></td>
				...
				<tr>
					...
					<td><input style="width: 90%;" type="text"
						name="powerbuttonshort_script"
						value="<?php echo htmlspecialchars($as->powerbutton['short_script']) ?>" />
					</td>
				...
				<tr>
					...
					<td style="width: 25%;"><select name="powerbuttonlong"
						style="width: 90%;">
							<option
								<?php if($as->powerbutton['longpress'] == 'myscript') echo 'selected'; ?>
								value='myscript'><?php echo _('Start my own Script') ?></option>
					</select></td>
				...
				<tr>
					...
					<td><input style="width: 90%;" type="text"
						name="powerbuttonlong_script"
						value="<?php echo htmlspecialchars($as->powerbutton['long_script']) ?>" />
					</td>
			...
			<input type="button" name="save" value="<?php echo _('save') ?>"
				onclick="document.getElementById('action').value='configurePowerButton';submit();" />
		</div>
		...
		<input type="button" id="checkUpgrade" name="checkUpgrade"
			onclick="document.getElementById('action').value='checkUpgrade';submit();"
			value="<?php echo _("Check Available Upgrades") ?>" /><br /> <input
			type="button" id="upgradeSystem" name="upgradeSystem"
			onclick="document.getElementById('action').value='upgradeSystem';submit();"
			value="<?php echo _("Upgrade Packages") ?>" /><br />		
		...
		<input type="button" id="installFlash" name="installFlash"
			onclick="document.getElementById('action').value='installFlash';submit();"
			value="<?php echo _("Install Pepperflash") ?>" /><br />
		...
		<input type="button" id="installFan" name="installFan"
			onclick="document.getElementById('action').value='installFan';submit();"
			value="<?php echo _("Install Fan silencer") ?>" /><br />
