...
?>	    	  
	...
	<?php }else { ?>
		...
		<br />				
		<?php echo _('Reinitialize Display (if screen is black or HDMI was not connected on startup)') ?><input
			type="checkbox" value="1" name="reinitx" /> <br /> <input
			type="button"
	...
	<?php } ?>
		<br />
		<br /> <b><?php echo str_replace('$SERVICENAME', $sp->viewname, _('Autostart $SERVICENAME')) ?>:</b><sup>#1</sup><br />
		<input type="checkbox"
		...
		<input id="downloadurl" type="text" value="" name="downloadurl"
		...
		<?php if($sp->getSystemUser() == 'pi'){ ?>
			&nbsp;&nbsp;<input type="button"
			value="<?php echo _('Upgrade Kodi to latest version') ?>"
			name="upgrade"
			onclick="document.getElementById('downloadurl').value='kodiupgradepi';document.getElementById('action').value='install';submit();" />
		<?php } ?>
		<br />
		<br /> <?php echo _('The installation may take some time depending on your internet connection. At first it downloads the package from given source and afterwards it installs the package.')?>			
		<br />
		<br />
		...
		<br /> <input id="addonurl" type="text" value="" name="addonurl"
			style="width: 200px;" /><br /> <select name="addonbox"
			onclick="copytodownloadurl(this);">
			<option value=""><?php echo _('Choose Addon')?></option>
			<option
				value="https://addonscriptorde-beta-repo.googlecode.com/files/repository.addonscriptorde-beta.zip">Amazon
				Instant Video (old XBMC version)</option>
			<option
				value="https://github.com/XLordKX/kodi/raw/master/zip/repository.xlordkx/repository.xlordkx-1.0.0.zip">Amazon
				Instant Video (new Kodi version)</option>
			<option
				value="http://cdn.max2play.com/kodi-15-pvr/max2play-u3-repository.zip">Kodi
				15 PVR-Addons Repository (for ODROID and Kodi version 15 ONLY!)</option>
			<option
				value="http://cdn.max2play.com/kodi-15-pvr/max2play-rpi-repository.zip">Kodi
				15 PVR-Addons Repository (for Raspberry Pi and Kodi version 15
				ONLY!)</option>
			<option value="kodi-platform-pi-jessie">Kodi-Platform Installation
				RPi Jessie (needed for PVR-Addons - DLL-Error fix)</option>
			<option
		...
		<input type="button"
			value="<?php echo _('Get Addon-Repository for Kodi / XBMC') ?>"
			name="getaddon"
			onclick="document.getElementById('action').value='getaddon';submit();" />
