...
class Start_Setup extends Service {
	 ...
	 */
	private function checkPicturesAndHeader($custom = ''){		
		if($custom != '' && !file_exists(APPLICATION_PATH.'/../public/addons/'.$custom.'/'.$custom.'-logo-450.jpg')){
			...
			$this->writeDynamicScript(array(
					...
					'cp '.dirname(__FILE__).'/../scripts/images/* '.APPLICATION_PATH.'/../public/addons/'.$custom.'/',
					'cp '.dirname(__FILE__).'/../scripts/custom.css '.APPLICATION_PATH.'/../public/custom.css',
					'cp '.dirname(__FILE__).'/../view/header_custom.php '.APPLICATION_PATH.'/view/header_custom.php'
			));
			$this->view->message[] = 'Created View-Files';
		}
	}
	 ...
	 */
	private function _savePurpose($purpose = ''){
		if($purpose == 'mpd'){
			// Install MPD Plugin
