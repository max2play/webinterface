...
<?php if($cs->view->installed) { ?>	
	...
	<form action="" method="get">
		...
		<p class="ui-state-default ui-corner-all"
			...
			<span class="ui-icon ui-icon-wrench"
				style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _t("Main tellows Settings - Connected to tellows: ") ?><?php echo $cs->tellows->registered ?></b>
		</p>
	...
	<br />
		...
		<table class="settings">
			<tr>
				...
				<td><select name="tellows_minscore">
						<option value="7"
							<?php if($cs->tellows->minscore == 7) echo 'selected' ?>>Score 7</option>
						<option value="8"
							<?php if($cs->tellows->minscore == 8) echo 'selected' ?>>Score 8</option>
						<option value="9"
							<?php if($cs->tellows->minscore == 9) echo 'selected' ?>>Score 9</option>
				</select></td>
			...
			<tr>
				...
				<td><input type="text" value="<?php echo $cs->tellows->apikey ?>"
					name="tellows_apikey" /></td>
		...
		<input type="button" value="<?php echo _t('save') ?>" name="save"
			onclick="document.getElementById('action').value='savetellows';submit();" />
		<br />
		<br />
		...
		<p class="ui-state-default ui-corner-all"
			...
			<span class="ui-icon ui-icon-gear"
				style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _t("SIP Settings for VOIP Setup - Register Status: ") ?><?php echo $cs->linphone->registered ?></b>
		</p>
		<table class="settings">
			<tr>
				<td><?php echo _t("VOIP-Host") ?></td>
				<td><input type="text" value="<?php echo $cs->linphone->host ?>"
					name="linphone_host" /></td>
				<td><?php echo _t("If you have a fritzbox the value is &quot;fritz.box&quot;") ?></td>
			</tr>
			<tr>
				<td><?php echo _t("VOIP-User") ?></td>
				<td><input type="text" value="<?php echo $cs->linphone->user ?>"
					name="linphone_user" /></td>
				<td><?php echo _t("If you have a fritzbox the value is problably &quot;620&quot;") ?></td>
			</tr>
			<tr>
				<td><?php echo _t("VOIP-Password") ?></td>
				<td><input type="text" value="<?php echo $cs->linphone->password ?>"
					name="linphone_password" /></td>
				<td><?php echo _t("This is your VOIP-Password.") ?></td>
			</tr>
			<tr>
				<td><?php echo _t("Audio File for Blacklist") ?></td>
				<td><select name="tellows_audiofile">
						<option value="0"
							<?php if($cs->tellows->audiofile == 0) echo 'selected'; ?>><?php echo _t("Hang up without Sound") ?></option>
						<option value="1"
							<?php if($cs->tellows->audiofile == 1) echo 'selected'; ?>><?php echo _t("Play: You are blocked") ?></option>
				</select></td>
		...
		<input type="button" value="<?php echo _('save') ?>" name="save"
			onclick="document.getElementById('action').value='savelinphone';submit();" />
		<br /> <br />
		<p class="ui-state-default ui-corner-all"
			style="padding: 4px; margin-bottom: 1em;">
			<span class="ui-icon ui-icon-wrench"
				style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _t("Modem for PSTN (analog) Connected: ") ?><?php echo $cs->view->modemconnected ?></b>
		</p>
		...
		<table class="settings">
			<tr>
				...
				<td><select name="hangup_type">
						<option value="0"
							<?php if($cs->modem->hangup_type == 0) echo 'selected'; ?>><?php echo _t("Do Nothing (not recommended)") ?></option>
						<option value="1"
							<?php if($cs->modem->hangup_type == 1) echo 'selected'; ?>><?php echo _t("Hangup (block call)") ?></option>
						<option value="2"
							<?php if($cs->modem->hangup_type == 2) echo 'selected'; ?>><?php echo _t("Answer as Fax-Machine") ?></option>
				</select></td>
				<td><?php echo _t("Choose how the callblocker will react on a blacklisted Caller (number on blacklist)") ?></td>
			</tr>
		</table>
		<input type="button" value="<?php echo _('save') ?>" name="save"
			onclick="document.getElementById('action').value='savemodemsettings';submit();" />
	<?php } ?>	
	</form>
	<br />
...
foreach ($cs->view->debug as $key => $debug) {
        echo "#### " . $key . " ####\n";
        echo $debug . " \n\n";
    }
    ?>
...
<?php }?>
	<form action="" method="get">
		...
		<input type="button"
			value="<?php echo _t('Update tellows Callblocker') ?>" name="update"
			onclick="document.getElementById('actionupdate').value='updateCallblocker';submit();" />
		&nbsp;&nbsp; <input type="button"
			value="<?php echo _t('Reboot Callblocker') ?>" name="reboot"
			onclick="document.getElementById('actionupdate').value='reboot';submit();" />
	</form>
