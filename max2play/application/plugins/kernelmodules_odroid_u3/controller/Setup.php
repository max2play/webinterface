	...
	private function _buildHifaceWithKernel($ajax = 0){
		...
		if($ajax == 0){
			...
			if($this->getProgressWithAjax('/opt/max2play/cache/build_kernel.txt', 1, 0)){
				$shellanswer = $this->writeDynamicScript(array($this->scriptPath."build_kernel.sh >> /opt/max2play/cache/build_kernel.txt &"));
			}
		}else{
	...
	private function _enableHifaceUSB($enable = false){		
		...
		if($enable){			
			if($this->view->hiface_enabled == 0){
				$this->writeDynamicScript(array("sed -i 's/^exit 0/insmod ".str_replace('/','\\/',$this->scriptPath)."snd-usb-hiface.ko\\n\\nexit 0/' /etc/rc.local"));
				$this->view->message[] = _('Added HiFace USB-Driver');
			}
		}else{
			if($this->view->hiface_enabled == 1){
				$this->writeDynamicScript(array("sed -i 's/^insmod ".str_replace('/','\\/',$this->scriptPath)."snd-usb-hiface.ko//' /etc/rc.local"));
