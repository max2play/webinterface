...
<div class="entry-content">
	...
	<p class="ui-state-default ui-corner-all"
		...
		<span class="ui-icon ui-icon-wrench"
			style="float: left; margin: -2px 5px 0 0;"></span> <b><?php echo _("Hiface USB-DAC: Status=").$kernelmodules_odroid_u3->view->hiface_enabled ?></b>
	</p>

	<form action="" method="get">
		<input type="hidden" id="action" name="action" value="" /> <input
			type="button" id="enableHifaceUSB"
			name="<?php echo _("Load Module to etc/rc.local") ?>"
			onclick="document.getElementById('action').value='enableHifaceUSB';submit();"
			value="<?php echo _("Load Module to etc/rc.local") ?>" />
		&nbsp;&nbsp;&nbsp; <input type="button" id="disableHifaceUSB"
			name="<?php echo _("Unload Module from etc/rc.local") ?>"
			onclick="document.getElementById('action').value='disableHifaceUSB';submit();"
			value="<?php echo _("Unload Module from etc/rc.local") ?>" /> <br />
		<br />
		...
		<input type="button" id="buildHifaceWithKernel"
			name="<?php echo _("Build HiFace-Module with latest Kernel") ?>"
			onclick="document.getElementById('action').value='buildHifaceWithKernel';submit();"
			value="<?php echo _("Build HiFace-Module with latest Kernel") ?>" />
...
foreach ($kernelmodules_odroid_u3->view->debug as $key => $debug) {
    echo "#### " . $key . " ####\n";
    echo $debug . " \n\n"; 
		 }?>
	</textarea>
